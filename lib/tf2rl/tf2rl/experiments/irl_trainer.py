...
class IRLTrainer(Trainer):
    ...
    def set_info(self):
        self.info = {}
        self.info["self._policy.n_warmup"] = self._policy.n_warmup
        self.info["self._max_steps"] = self._max_steps
        f = open("log/log.txt", "w")
        self.mylogger = f
        return self.info

    def __call__(self):
        print("Info: ", self.set_info())
        total_steps = 0
        ...
        while total_steps < self._max_steps:
            ...
            while total_steps < self._max_steps:
                ...
                print("ep_steps:", episode_steps, "action: ", list(action), "reward: ", reward, file=self.mylogger)
                total_steps += 1
                tf.summary.experimental.set_step(total_steps)
